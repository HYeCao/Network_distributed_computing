...
public class HttpClient {
	...
	public void processPutRequest(String request) throws Exception {
		//=======start your job here============//
		//find file name
		//first split is cmd, second is filename
		...
		request += "Accept: */*"+CRLF;
		request += "Accept-Language: zh-cn"+CRLF;
		request += "Content-Length: " + file.length() + CRLF + CRLF;


		DataInputStream dis = new DataInputStream(new BufferedInputStream(new FileInputStream(file)));
		byte[] mes = new byte[(int)file.length()];
		dis.read(mes);

		buffer = request.getBytes();
		byte[] send = new byte[request.getBytes().length + mes.length];
		for (int i = 0 ; i < request.getBytes().length ; ++i){
			send[i] = buffer[i];
		}
		for(int i = 0 ; i < mes.length ;++i){
			send[request.getBytes().length + i] = mes[i];
		}
...
//		System.out.println("H:"+header.toString());
...
//		System.out.println("B:"+response.toString());
	}
